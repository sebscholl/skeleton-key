...
RSpec.describe "Solana BIP44 Deep Vector Compliance" do
  let(:vectors) { load_fixture("vectors/solana/bip44_deep_golden_master") }
  let(:keyring) { SkeletonKey::Keyring.new(seed: vectors["bip39_seed"]) }

  describe "Address Derivation" do
    it "derives the expected deep hardened Solana addresses and keys for each branch" do
      fixture_branches(vectors).each do |branch|
        ...
        branch["addresses"].each do |addr|
          ...
          derived = solana_account.address(change: change, index: index)
